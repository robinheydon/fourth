    ...
    for (1.., test_functions) |i, test_fn| {
        ...
        if (test_fn.func()) |_| {
            if (std.testing.allocator_instance.deinit() == .leak) {
                leaked += 1;
                std.debug.print("\x1b[31;1m{d:3}/{d:3} LEAK \"{}\"\x1b[0m\n", .{
                    i,
                    test_functions.len,
                ...
                passed += 1;
                happy_writer.print("\x1b[32m{d:3}/{d:3} OK   \"{}\"\x1b[0m\n", .{
                    i,
                    test_functions.len,
                    std.zig.fmtEscapes(test_fn.name),
                }) catch {};
            ...
            if (std.testing.allocator_instance.deinit() == .leak) {
                ...
                switch (err) {
                    error.SkipZigTest => {
                        skipped += 1;
                        happy_writer.print("\x1b[37;3m{d:3}/{d:3} SKIP \"{}\"\x1b[0m\n", .{
                            i,
                            test_functions.len,
                            std.zig.fmtEscapes(test_fn.name),
                        }) catch {};
                        ...
                        failed += 1;
                        std.debug.print("\x1b[31;1m{d:3}/{d:3} FAIL \"{}\" {s}\x1b[0m\n", .{
                            i,
                            test_functions.len,
                            std.zig.fmtEscapes(test_fn.name),
                            @errorName(err),
