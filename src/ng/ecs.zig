...
pub fn entity_iterator() EntityIterator {
    return .{};
}
...
pub const EntityIterator = struct {
    ...
    pub fn next(self: *EntityIterator) ?Entity {
        ...
        if (idx < generations.items.len) {
            const gen = generations.items[self.idx];
            self.idx += 1;
            return .{ .gen = gen, .idx = idx };
        }
        return null;
    }
};
...
pub fn component_iterator() ComponentIterator {
    return .{};
}
...
pub const ComponentIterator = struct {
    ...
    pub fn next(self: *ComponentIterator) ?ComponentInfo {
        const index = self.index;
        if (index < components.items.len) {
            self.index += 1;
            return components.items[index];
        }
        return null;
    }
};
...
pub fn get_component(typeid: usize) ?*const ComponentInfo {
    if (typeid < components.items.len) {
        return &components.items[typeid];
    }
    return null;
}
